...
class Post::UpdatePublishedTest < ActionDispatch::IntegrationTest
  test "A user can unpublish a post on Dev.to, and the 'devto_draft' attribute will be modified if the response is successful" do
    user = users(:one)
    post = user.posts.create(title: "title", md_content: "example", devto_draft: false, devto_id: "123456")
    platform = "Dev.to"
    published = false
    sign_in user
    ...
    assert_changes -> { post.devto_draft } do
      VCR.use_cassette("devto_update_published") do
        post "/posts/#{post.id}/update_published"
